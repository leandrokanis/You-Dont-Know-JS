...
2. _the newly constructed object is_ `[[Prototype]]`_-linked_
...
Whoa! `bar` is hard-bound against `obj1`, but `new bar(3)` did **not** change `obj1.a` to be `3` as we would have expected. Instead, the _hard bound_ \(to `obj1`\) call to `bar(..)` _**is**_ able to be overridden with `new`. Since `new` was applied, we got the newly created object back, which we named `baz`, and we see in fact that `baz.a` has the value `3`.
